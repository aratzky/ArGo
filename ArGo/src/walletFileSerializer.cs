    ...
    internal class walletFileSerializer
    {
            ...
            string network, Addresses addresses)
        {
            ...
            if (!File.Exists(TransactionsPath))
            {
                ...
                File.WriteAllText(TransactionsPath, JsonConvert.SerializeObject(data));
            }


            var content =
                JsonConvert.SerializeObject(
                    new walletFileSerializer(encryptedBitcoinPrivateKey, chainCode, network, TransactionsPath),
                    Formatting.Indented);
